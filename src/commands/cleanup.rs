...
pub fn execute(older_than: Option<u32>, force: bool) -> Result<()> {
    ...
    if !force {
        ...
        for idx in &selected {
            ...
            println!("  • {}/{}", wt.state.project_name.blue(), wt.state.name.green());
        }
...
fn delete_worktree(state: &WorktreeState) -> Result<()> {
    ...
    if close_script.exists() {
        ...
        let _ = scripts::execute_script_ignore_errors(&close_script, &env);
    }

    // Deallocate ports
    let _ = ports::deallocate(&state.allocation_key);
    ...
    match git::remove_worktree(&state.original_dir, &state.worktree_dir, true) {
        ...
        Err(_) => {
            ...
            if std::fs::remove_dir_all(&state.worktree_dir).is_ok() {
                println!("    {} Removed (manually)", "✓".green());
            } else {
