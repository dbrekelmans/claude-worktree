...
pub fn execute(defaults: bool, no_scripts: bool) -> Result<()> {
    ...
    println!("  {} {}", "Created".green(), ".worktree/settings.json");
    ...
    println!();
    println!("Next steps:");
    println!("  1. Review and customize the scripts in {}", ".worktree/".cyan());
...
fn prompt_settings() -> Result<Settings> {
    ...
    let input = read_line()?;
    if !input.is_empty() {
        settings.port_count = input
            .parse()
            .context("Invalid port count")?;
    }
    ...
    let input = read_line()?;
    if !input.is_empty() {
        settings.port_range_start = input
            .parse()
    ...
    print!(
        ...
        if settings.auto_launch_terminal { "y" } else { "n" }.to_string().dimmed()
    );
    io::stdout().flush()?;
...
fn generate_scripts(config_dir: &std::path::Path, project_dir: &std::path::Path) -> Result<()> {
    ...
    println!("  {} {}", "Created".green(), ".worktree/setup.sh");
    println!("  {} {}", "Created".green(), ".worktree/run.sh");
    println!("  {} {}", "Created".green(), ".worktree/stop.sh");
    println!("  {} {}", "Created".green(), ".worktree/close.sh");
