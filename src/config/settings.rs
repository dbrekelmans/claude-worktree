...
impl MergedSettings {
    /// Load and merge settings from project config directory
    ...
    pub fn get_worktree_base_dir(&self, project_name: &str) -> PathBuf {
        match &self.worktree_dir {
            Some(dir) => dir.clone(),
            None => paths::global_worktrees_dir().join(project_name),
        }
    }
}
