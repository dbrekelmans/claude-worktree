...
fn parse_generated_scripts(output: &str) -> Result<GeneratedScripts> {
    let mut scripts = GeneratedScripts::default();

    // Try to extract scripts between markers
    scripts.setup_sh = extract_script(output, "setup.sh")
        .unwrap_or_else(default_setup_script);
    scripts.run_sh = extract_script(output, "run.sh")
        .unwrap_or_else(default_run_script);
    scripts.stop_sh = extract_script(output, "stop.sh")
        .unwrap_or_else(default_stop_script);
    scripts.close_sh = extract_script(output, "close.sh")
        .unwrap_or_else(default_close_script);
